...
struct MeshMaterial
{
    ...
    D3D12_CPU_DESCRIPTOR_HANDLE Texture(MaterialTextures texType) const
    {
        Assert_(uint64(texType) < uint64(MaterialTextures::Count));
        Assert_(Textures[uint64(texType)] != nullptr);
        return Textures[uint64(texType)]->SRV.CPUHandle;
    }
...
public:
    ...
    void InitCommon(const MeshVertex* vertices, const uint16* indices, uint64 vbAddress, uint64 ibAddress, uint64 vtxOffset, uint64 idxOffset);
...
public:
    ...
    D3D12_CPU_DESCRIPTOR_HANDLE MaterialTextureDescriptors() const { return descriptorHeap.CPUStart; }
...
protected:
    ...
    LinearDescriptorHeap descriptorHeap;
};
