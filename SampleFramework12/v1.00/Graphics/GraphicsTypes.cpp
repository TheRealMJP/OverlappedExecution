...
DescriptorHandle LinearDescriptorHeap::Allocate(uint64 count)
{
    ...
    int64 idx = InterlockedAdd64(&Allocated, count) - count;
    Assert_(uint64(idx) < NumDescriptors);
...
void Buffer::Initialize(uint64 size, uint64 alignment, bool32 dynamic, BufferLifetime lifetime, bool32 allowUAV,
                        const void* initData, D3D12_RESOURCE_STATES initialState, ID3D12Heap* heap, uint64 heapOffset,
                        const wchar* name)
{
    Assert_(size > 0);
    Assert_(alignment > 0);
...
void StructuredBuffer::Initialize(const StructuredBufferInit& init)
{
    ...
    InternalBuffer.Initialize(Stride * NumElements, Stride, init.Dynamic, init.Lifetime, init.CreateUAV,
...
void RawBuffer::Initialize(const RawBufferInit& init)
{
    ...
    InternalBuffer.Initialize(Stride * NumElements, Stride, init.Dynamic, init.Lifetime, init.CreateUAV, init.InitData,
