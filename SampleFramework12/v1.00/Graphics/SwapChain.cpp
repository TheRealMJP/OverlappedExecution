...
void SwapChain::Shutdown()
{
    for(uint64 i = 0; i < NumBackBuffers; ++i)
    {
        ...
        DX12::RTVDescriptorHeap.Free(backBuffers[i].RTV);
    }
...
void SwapChain::AfterReset()
{
    ...
    for(uint64 i = 0; i < NumBackBuffers; i++)
    {
        backBuffers[i].RTV = DX12::RTVDescriptorHeap.Allocate();
        ...
        DX12::Device->CreateRenderTargetView(backBuffers[i].Texture.Resource, &rtvDesc, backBuffers[i].RTV.CPUHandle);
