...
void Skybox::Initialize()
{
    ...
    vertexShader = CompileFromFile(shaderPath.c_str(), "SkyboxVS", ShaderType::Vertex, ShaderProfile::SM51);
    pixelShader = CompileFromFile(shaderPath.c_str(), "SkyboxPS", ShaderType::Pixel, ShaderProfile::SM51);
    ...
    vertexBuffer.Initialize(sizeof(verts), DX12::VertexBufferAlignment,
                            false, BufferLifetime::Persistent, false, verts,
                            D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER,
                            nullptr, 0, L"Skybox Vertex Buffer");
    ...
    indexBuffer.Initialize(sizeof(indices), DX12::IndexBufferAlignment,
                           false, BufferLifetime::Persistent, false, indices,
                           D3D12_RESOURCE_STATE_INDEX_BUFFER,
                           nullptr, 0, L"Skybox Index Buffer");

    // Create the VS constant buffer
    vsConstantBuffer.Initialize(BufferLifetime::Temporary);

    // Create the PS constant buffer
    psConstantBuffer.Initialize(BufferLifetime::Temporary);
}
...
void Skybox::Shutdown()
{
    ...
    vsConstantBuffer.Shutdown();
    psConstantBuffer.Shutdown();
...
{
    // Set the constants
    vsConstantBuffer.Data.View = view;
    vsConstantBuffer.Data.Projection = projection;
    vsConstantBuffer.Upload();

    psConstantBuffer.Data.Scale = scale;
    psConstantBuffer.Upload();
    ...
    // Bind descriptors and constant buffers
    D3D12_CPU_DESCRIPTOR_HANDLE psResources[1] = { environmentMap->SRV.CPUHandle };

    DX12::BindShaderResources(cmdList, 0, ArraySize_(psResources), psResources, CmdListMode::Graphics, ShaderResourceType::SRV_UAV_CBV);
    vsConstantBuffer.SetAsGfxRootParameter(cmdList, 1);
    psConstantBuffer.SetAsGfxRootParameter(cmdList, 2);

    // Set vertex/index buffers
    D3D12_VERTEX_BUFFER_VIEW vbView = { };
    vbView.BufferLocation = vertexBuffer.GPUAddress;
    vbView.SizeInBytes = uint32(vertexBuffer.Size);
    vbView.StrideInBytes = sizeof(Float3);
                                  ...
                                  const Float4x4& projection, const Texture* environmentMap, Float3 scale)
{
    ...
    psConstantBuffer.Data.CosSunAngularRadius = 0.0f;
                       ...
                       bool enableSun, const Float3& scale)
{
    ...
    psConstantBuffer.Data.SunDirection = skyCache.SunDirection;
    psConstantBuffer.Data.Scale = scale;
    if(enableSun)
    {
        ...
        psConstantBuffer.Data.SunColor = Float3::Clamp(sunColor, 0.0f, FP16Max);
        psConstantBuffer.Data.CosSunAngularRadius = std::cos(DegToRad(skyCache.SunSize));
    }
    else
    {
        psConstantBuffer.Data.SunColor = 0.0f;
        psConstantBuffer.Data.CosSunAngularRadius = 0.0f;
    }
